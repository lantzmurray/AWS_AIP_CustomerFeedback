...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Save validation results to validation-results folder
        validation_key = key.replace('text_reviews/', 'validation-results/').replace('.txt', '.json').replace('.json', '_validation.json')
        
        # Create enhanced validation result with original content
        enhanced_validation_result = {
            'file_name': key,
            'customer_id': extract_customer_id_from_filename(key),
            'text_content': text,
        ...
        s3_client.put_object(
            Bucket=bucket,
            Key=validation_key,
            Body=json.dumps(enhanced_validation_result, default=str),
            ContentType='application/json',
            Metadata={
                'original_file': key,
                'validation_score': str(validation_results['quality_score']),
                'validation_timestamp': datetime.now().isoformat()
            }
        )
        
        # Also save to processed bucket for integration with processing pipeline
        processed_validation_key = validation_key.replace('validation-results/', 'processed/text_reviews/')
        s3_client.put_object(
            Bucket=bucket.replace('-raw', '-processed'),
            Key=processed_validation_key,
            Body=json.dumps(enhanced_validation_result, default=str),
            ContentType='application/json',
            Metadata={
                'original_file': key,
    ...
    try:
        ...
        if len(parts) >= 2:
            ...
            if file_part.startswith('review_'):
                return file_part.replace('review_', '').replace('.txt', '').replace('.json', '')
        return 'unknown'
    except Exception:
        return 'unknown'
